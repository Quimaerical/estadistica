...
class _StatAppState extends State<StatApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MenuScreen(onThemeToggle: _toggleTheme, isDarkMode: _themeMode == ThemeMode.dark),
    );
  }
}
...
class MenuScreen extends StatelessWidget {
  ...
  const MenuScreen({super.key, required this.onThemeToggle, required this.isDarkMode});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Herramientas Estadísticas', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ...
      body: ListView(
        ...
        children: [
          _MenuButton(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DescriptiveStatsScreen())),
          ),
          ...
          _MenuButton(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DistributionCalculatorScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            icon: Icons.science,
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HypothesisTestScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const GroupedDataScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MomentsScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            icon: Icons.school,
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TheoreticalDistributionsScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MGFGuideScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            icon: Icons.casino,
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MonteCarloScreen())),
          ),
          const SizedBox(height: 16),
          _MenuButton(
            icon: Icons.list_alt,
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DistributionsGuideScreen())),
          ),
        ],
      ),
    );
...
class MathUtils {
  ...
  static double variance(List<double> data, {bool isSample = true}) {
    ...
    double m = mean(data);
    double sumSquaredDiff = data.map((x) => math.pow(x - m, 2)).reduce((a, b) => a + b).toDouble();
  ...
  static double skewness(List<double> data, {bool isSample = true}) {
    ...
    double sumCubedDiff = data.map((x) => math.pow((x - m) / s, 3)).reduce((a, b) => a + b).toDouble();
    
    if (isSample) {
      return (n / ((n - 1) * (n - 2))) * sumCubedDiff; // Fisher-Pearson adjusted
    } else {
  ...
  static double kurtosis(List<double> data, {bool isSample = true}) {
    ...
    if (s == 0) return 0;
    double n = data.length.toDouble();
    double sumQuarticDiff = data.map((x) => math.pow((x - m) / s, 4)).reduce((a, b) => a + b).toDouble();
  ...
  static double skewnessGrouped(List<Map<String, dynamic>> intervals, double mean, double stdDev) {
  ...
  static double kurtosisGrouped(List<Map<String, dynamic>> intervals, double mean, double stdDev) {
    if (intervals.isEmpty || stdDev == 0) return 0;
    double n = intervals.fold(0, (sum, item) => sum + (item['freq'] as int));
  ...
  static double chi2Ppf(double p, double k) {
    return _solvePpf((val) => chi2Cdf(val, k), p, 0, 200); // Chi2 is always positive
  }
  ...
  static double fPpf(double p, double d1, double d2) {
    return _solvePpf((val) => fCdf(val, d1, d2), p, 0, 200); // F is always positive
  }
  ...
  // --- UTILS ---
  static double _solvePpf(double Function(double) cdf, double targetP, double min, double max) {
  ...
  static double fPdf(double x, double d1, double d2) {
    if (x <= 0) return 0;
    double num = math.sqrt(math.pow(d1 * x, d1) * math.pow(d2, d2) / math.pow(d1 * x + d2, d1 + d2));
  ...
  static Map<String, dynamic> fitDistribution(List<double> data) {
    ...
    return {
      'type': best,
      'params': best == 'Normal' ? {'mean': mean, 'stdDev': stdDev} :
                best == 'Exponencial' ? {'lambda': lambda} :
                {'min': min, 'max': max}
    };
  }
}
...
class BoxPlotPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Whiskers
    canvas.drawLine(Offset(xMin, yCenter), Offset(xQ1, yCenter), paint); // Left whisker
    canvas.drawLine(Offset(xQ3, yCenter), Offset(xMax, yCenter), paint); // Right whisker
  ...
  void drawText(Canvas canvas, String text, Offset offset) {
    TextSpan span = TextSpan(style: TextStyle(color: isDark ? Colors.grey[400] : Colors.black54, fontSize: 10), text: text);
...
class _DescriptiveStatsScreenState extends State<DescriptiveStatsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Análisis Descriptivo', style: TextStyle(color: Colors.white)), backgroundColor: Colors.indigo),
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        const Text("Entrada de Datos", style: TextStyle(fontWeight: FontWeight.bold)),
                        Switch(value: _isGroupedInput, onChanged: (v) => setState(() => _isGroupedInput = v)),
                      ],
                    ),
                    if (!_isGroupedInput)
                      TextField(
                        ...
                        decoration: const InputDecoration(
                          hintText: "Ej: 10.5, 20, 15.2 (separado por comas o espacios)",
                      ...
                      Row(
                        children: [
                          Expanded(child: TextField(controller: _valCtrl, decoration: const InputDecoration(labelText: "Valor"), keyboardType: TextInputType.number)),
                          ...
                          IconButton(onPressed: _addGroupedPoint, icon: const Icon(Icons.add_circle, color: Colors.indigo))
                        ],
                      ),
                  ],
                ),
  ...
  List<FlSpot> _getNormalCurveSpots(double mean, double stdDev, List<double> data) {
    if (stdDev == 0) return [];
  ...
  void _showFreqTable(BuildContext context, List<double> data) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          children: [
            const Text("Tabla de Frecuencias", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Expanded(
              child: ListView(
                children: [
...
class DistributionCalculatorScreen extends StatefulWidget {
  ...
  @override
  State<DistributionCalculatorScreen> createState() => _DistributionCalculatorScreenState();
}

class _DistributionCalculatorScreenState extends State<DistributionCalculatorScreen> {
  ...
  final TextEditingController _valCtrl = TextEditingController(); // Can be x or p
  ...
  String _compute(double? p1, double? p2, double val) {
    bool isDiscrete = ['Binomial', 'Poisson', 'Geométrica'].contains(_selectedDist);
    // Helper to check standard PPF support (inverse)
    bool supportsInverse = !isDiscrete && !['Exponencial', 'Uniforme'].contains(_selectedDist); 
    ...
    if (_targetMode == 1) {
      ...
      if (!supportsInverse) return "Cálculo inverso no soportado para $_selectedDist";
      if (_relation == 'eq') return "No se puede calcular inverso para igualdad exacta.";
      ...
      switch (_selectedDist) {
        case 'Normal': x = MathUtils.normalPpf(targetP, p1 ?? 0, p2 ?? 1); break;
        case 't-Student': x = MathUtils.tPpf(targetP, p1 ?? 1); break;
        case 'Chi-Cuadrado': x = MathUtils.chi2Ppf(targetP, p1 ?? 1); break;
        case 'F-Snedecor': x = MathUtils.fPpf(targetP, p1 ?? 1, p2 ?? 1); break;
        case 'Gamma': x = MathUtils.gammaPpf(targetP, p1 ?? 1, p2 ?? 1); break;
      }
  ...
  String _computeDiscreteP(double? p1, double? p2, double val) {
    ...
    double getCdf(int k) {
      switch (_selectedDist) {
        case 'Binomial': return MathUtils.binomialCdf(k, (p1 ?? 10).toInt(), p2 ?? 0.5);
        case 'Poisson': return MathUtils.poissonCdf(k, p1 ?? 1);
        case 'Geométrica': return MathUtils.geometricCdf(k, p1 ?? 0.5);
        default: return 0;
      }
    }
    ...
    double getPmf(int k) {
       switch (_selectedDist) {
        case 'Binomial': return MathUtils.binomialPmf(k, (p1 ?? 10).toInt(), p2 ?? 0.5);
        case 'Poisson': return MathUtils.poissonPmf(k, p1 ?? 1);
        case 'Geométrica': return MathUtils.geometricPmf(k, p1 ?? 0.5);
        default: return 0;
      }
    }
  ...
  String _getP1Label() {
    switch (_selectedDist) {
      ...
      case 't-Student': return "Grados de libertad (df)";
      case 'Chi-Cuadrado': return "Grados de libertad (k)";
      case 'F-Snedecor': return "Grados libertad num (d1)";
      case 'Gamma': return "Forma (α)";
  ...
  String? _getP2Label() {
    switch (_selectedDist) {
      ...
      case 'F-Snedecor': return "Grados libertad den (d2)";
      case 'Gamma': return "Escala (β)";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Calculadora Distribuciones'), backgroundColor: Colors.indigo, foregroundColor: Colors.white),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          children: [
            DropdownButtonFormField<String>(
              value: _selectedDist,
              items: ['Normal', 't-Student', 'Chi-Cuadrado', 'F-Snedecor', 'Gamma', 'Binomial', 'Poisson', 'Exponencial', 'Uniforme', 'Geométrica']
            ...
            // Mode Selection
            const Text("Tipo de Cálculo", style: TextStyle(fontWeight: FontWeight.bold)),
            Row(
              children: [
                Expanded(
                  child: RadioListTile(
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: DropdownButtonFormField<String>(
                    value: _relation,
                    items: const [
                       DropdownMenuItem(value: 'eq', child: Text("Igual a (X = x)")),
                       DropdownMenuItem(value: 'le', child: Text("Menor o Igual (X ≤ x)")),
                       DropdownMenuItem(value: 'lt', child: Text("Menor que (X < x)")),
                       DropdownMenuItem(value: 'ge', child: Text("Mayor o Igual (X ≥ x)")),
                       DropdownMenuItem(value: 'gt', child: Text("Mayor que (X > x)")),
                    ],
                ...
                Expanded(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: _targetMode == 0 ? "Valor X" : "Probabilidad P", 
            ...
            if (_result.isNotEmpty)
              Container(
                ...
                child: Text(_result, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.indigo), textAlign: TextAlign.center),
              )
          ],
        ),
      ),
...
class _HypothesisTestScreenState extends State<HypothesisTestScreen> {
  ...
  void _runTest() {
    ...
    double pValue = 2 * (1 - MathUtils.tCdf(tStat.abs(), df.toDouble())); // Two-tailed
...
""";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Prueba t (1 muestra)'), backgroundColor: Colors.indigo, foregroundColor: Colors.white),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const Text("Datos de la Muestra", style: TextStyle(fontWeight: FontWeight.bold)),
            TextField(controller: _dataCtrl, maxLines: 3, decoration: const InputDecoration(hintText: "Ej: 10, 12, 11.5", border: OutlineInputBorder())),
            const SizedBox(height: 16),
            const Text("Parámetros de la Prueba", style: TextStyle(fontWeight: FontWeight.bold)),
            Row(
              children: [
                Expanded(child: TextField(controller: _muCtrl, decoration: const InputDecoration(labelText: "Media Hipotética (H0)", border: OutlineInputBorder()), keyboardType: TextInputType.number)),
                const SizedBox(width: 10),
                Expanded(child: TextField(controller: _alphaCtrl, decoration: const InputDecoration(labelText: "Significancia (α)", border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              ],
            ),
            const SizedBox(height: 20),
            ElevatedButton(
